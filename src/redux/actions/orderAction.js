...
export const CreateOrderApi = (post, cb,) => async (dispatch, getState) => {
  ...
  // console.log("CreateOrderApi postdata : ", post)
  cb && cb(false, '', true);
  http.post(url, post)
    .then(async response => {
...
export const UpdatetransactionIdApi = (orderId, transactionId, cb) => async (dispatch, getState) => {
  const { userstate } = getState().blacklist;
  const url = userstate == "Services" ? `updateTransactionId/${orderId}` : `eCommerce/updateTransitionId/${orderId}`

  // console.log("UpdatetransactionIdApi enter : ", orderId, transactionId)

  cb && cb(false, true);
  http.put(url, { "transactionId": transactionId })
    .then(async response => {
...
export const CancelBookingApi = (orderId, postData, cb) => async (dispatch, getState) => {
  const { getuser } = getState().auth;
  const { userstate } = getState().blacklist;
  const url = userstate == "Services" ? `cancelOrder/${orderId}/${getuser?._id}` : `eCommerce/cancelOrder/${orderId}`
...
export const GetAllOrdersApi = (page, cb) => async (dispatch, getState) => {
  ...
  const { userstate } = getState().blacklist;
  const url = userstate == "Services" ? `getAllOrderByCostomerId/${getuser?._id}` : `eCommerce/getOrderByCustomerId/${getuser?._id}`
...
export const GetOrderByIdApi = (id, pId, navigation, cb, uri, type) => async (dispatch, getState) => {
  const { userstate } = getState().blacklist;
  const url = userstate == "Services" ? `getOrderByOrderId/${id}` : `eCommerce/getByOrderId/${id}`

  cb && cb(true);
  http.get(uri ? uri : url)
    .then(async response => {
      ...
      if (response.data.success) {
        ...
        if (type == "Services") {
          navigation && navigation.navigate("Invoice", { data: response?.data?.data, pId: pId || response?.data?.data?.product?.[0]?._id })
        } else {
          navigation && navigation?.navigate('OrderDetail', { pId: pId || response?.data?.data?.product?.[0]?._id });
        }

        cb && cb(false, "success");
      } else {
        if (type == "Services") {
          navigation && navigation.navigate("Invoice", { data: response?.data?.data, pId: pId })
        } else {
...
export const SendReturnRequestApi = (pId, orderId, navigation, cb) => async (dispatch, getState) => {
  // ${getuser?._id}

  cb && cb(true)
  http.put(`eCommerce/returnRequestOrde/${orderId}?productId=${pId}`)
    .then(async response => {
      if (response.data.success) {
        dispatch(GetAllOrdersApi())
        cb && cb(false)
...
console.log('OpenPhonepayApi postData',postData)
  cb && cb(true)
  http.post(`PhonePayGateway`, postData)
    .then(async response => {
      console.log("PhonePayGateway res : ", response?.data, response?.data?.data?.data?.instrumentResponse?.redirectInfo);
      if (response?.data?.success) {
    ...
    .catch(error => {
      cb && cb(false)
      console.log("PhonePayGateway error ..................: ", error.response?.data)
...
export const CheckTransactionStatusApi = (id, cb) => async (dispatch, getState) => {


  cb && cb(true)
  http.get(`PhonePayGatewayCheckStatus?merchantTransactionId=${id}`)
    .then(async response => {
