...
const Home = ({
      ...
      switch (remoteMessage?.data?.type) {
        case 'ECOM_ORDERED':
          GetOrderByIdApi(
            ...
            navigation,
            null,
            `eCommerce/getByOrderId/${remoteMessage?.data?.orderId}`,
          );
          break;
        case 'ORDERED':
  ...
  const formatName = name => {
    if (!name) return '';
    return name
      .replace(/Funtional/g, 'Functional')
      .replace(/Mahcine/g, 'Machine')
      .trim();
  };
        ...
        <ScrollView
            ...
            <View style={styles.service_row}>
              {getHomedata?.category?.map((item, index) => (
                <Services
                  ...
                  onPress={() =>
                    GetParentCategoryApi(item?._id, navigation, data =>
                      setLoading(data),
                    )
                  }
                />
          ...
          {getHomedata?.homeCategoryCart?.map((item, index) => (
            <ServiceMore
              ...
              onPress={() => {
                GetParentCategoryApi(item?.pCategory, navigation, data =>
                  setLoading(data),
                );
              }}
