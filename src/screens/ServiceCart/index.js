...
const ServiceCart = ({
  ...
  getCoupon,
  getaddressbyuserid,
  GetAddressByUserIdApi, customerId,
  userstate, getuser, GetAllCouponApi,
  ...
  const appliedCoupon = getCoupon?.find(i => i?.couponCode == getallcart?.billDetail?.couponCode)
    ...
    if (token) {
      if (defaultAddress) {
        if (userstate == "Services") {
          navigation.navigate('SetTime', { data: { ...postData, "addressId": defaultAddress?._id } })
        } else if (userstate == "Product") {
          navigation.navigate('Payment', { data: { ...postData, "addressId": defaultAddress?._id } })
        }
      } else {
        RNToasty.Normal({
      ...
      {loading1 ?
        ...
        getallcart?.cartData?.[0] ?
          <>
            <ScrollView
              ...
              <View>
                <FlatList
                  ...
                  renderItem={({ item, index }) => (
                    <Cart
                      key={item?._id}
                      mt={index == 0 ? SIZES.height * .01 : 0}
                      disabled={true}
                      id={item?._id}
                      img={item?.productId?.thumnail ? { uri: http2 + item?.productId?.thumnail } : images?.no_image}
              ...
              <TouchableOpacity activeOpacity={0.5}
                ...
                style={styles.t1}>
                <Text style={styles.text1}>{userstate == "Services" ? "ADD MORE SERVICE" : "ADD MORE ITEMS"}</Text>
              </TouchableOpacity>

              <PriceDetail
                total={getallcart?.billDetail?.orderTotal}
                price={getallcart?.billDetail?.netAmount}
                tax={getallcart?.billDetail?.taxPercentage ? `${getallcart?.billDetail?.taxPercentage}%` : null}
                taxAmt={getallcart?.billDetail?.taxAmount ? `${getallcart?.billDetail?.taxAmount?.toFixed(2)}` : null}
              ...
              {token &&
                <>
                  ...
                  <TouchableOpacity activeOpacity={0.5} style={[styles.offer_btn,]} onPress={() => navigation?.navigate("Coupons")}>
                    <View style={styles.row}>
                      <Image source={icons.productdiscount} style={styles.img5} />
                      <Text style={[styles.text22, { width: width * 0.74 }]}>
                        {appliedCoupon ? `Save ${appliedCoupon?.couponPercent}% on this order` : "View best available offers"}
                      </Text>
                    </View>
                    <Icons name={"right"} size={SIZES.width * .05} color={COLORS.black} />
                  </TouchableOpacity>
                </>
              }
            ...
            {userstate == "Services" &&
              <TouchableOpacity activeOpacity={0.5} style={styles.checkout_btn}
            ...
            {userstate == "Product" &&
              <View style={styles.btn_box}>
                <View>
                  <Text style={styles.text13}>₹{getallcart?.billDetail?.orderTotal}</Text>
          ...
          <View style={styles.innerContainer}>
            <Text style={styles.topText}>Your Shopping Cart Is Empty.</Text>
            <Image source={images.cartEmpty} style={styles.image} />
            <Button t1="Shop Now" t2={{ width: SIZES.width * 0.45, alignSelf: 'center' }}
              onPress={() => navigation.navigate("Home")}
            // onPress={() => navigation?.navigate("Booking", { screen: "Completed" })}
            />
          </View>
      }
      <LoginModal visible={isModalVisible} onPress={() => { setModalVisible(false), navigation?.navigate("SignIn") }} />
    </View>
  );
};
