...
import { GetAllReviewApi, GetByProductIdApi } from '../../redux/actions/categoryAction';
...
const TabButton = ({ children, active, onPress, ml, mr, }) => {
  return (
    <TouchableOpacity activeOpacity={0.5}
      onPress={onPress}
      style={[
        styles.t3, ml && { marginLeft: ml }, mr && { marginRight: mr },
        active && {
...
const ServiceDetail = ({
  navigation, getallcart,
  GetByProductIdApi, GetAllReviewApi,
  getbyProductid, customerId, token,
  CreateCartApi, userstate,
  getuser,
}) => {
  ...
  console.log("product loading : ", loading, getbyProductid?.data?._id, getbyProductid?.data?.stock, getbyProductid?.data?.warranty )
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {getbyProductid?.data?.images &&
          <View style={styles.box5}>
            <HomeSlider
              ...
              imageKey={'url'} imageStyle={styles.sliderImage}
            />
          </View>
        }
        <View style={styles.innerContainer}>
          ...
          {getbyProductid?.data?.cityId?.cityName &&
            <View style={styles.row}>
              <Image source={images.HomeLocation} style={styles.locationstyle} />
          ...
          <View style={styles.row}>
            ...
            {userstate == "Services" && <Text style={styles.text6}>/ Visit</Text> }
          </View>
          {getbyProductid?.data?.warranty &&
            <View style={styles.row}>
              ...
              <Text style={styles.text3}> {getbyProductid?.data?.warranty == 0 ? 'No Warranty' : getbyProductid?.data?.warranty > 1 ? `${getbyProductid?.data?.warranty} /day's` : `${getbyProductid?.data?.warranty} /day`}</Text>
            </View>
          }
        ...
        <ScrollView horizontal>
          <View style={styles.box18}>
            {tablist?.map((item, index) => (
              <TabButton ml={index == 0 && width * .04} mr={index == tablist?.length - 1 ? width * .04 : 0} active={switchs == item?.type ? true : false} onPress={() => setSwitchs(item.type)}>{item.title}</TabButton>
            ))}
          </View>
        </ScrollView>
        <View style={styles.innerContainer}>
          ...
          {switchs == "include" && getbyProductid?.data?.include && (
          ...
          {switchs == "exclude" && getbyProductid?.data?.exclude && (
          ...
          {switchs == "additional" && getbyProductid?.data?.additional[0]?.url && (
            <View>
        ...
        {switchs == "review" && getbyProductid?.review?.[0] &&
          <ScrollView horizontal={true}>
            <View style={styles.box}>
              <FlatList
                data={getbyProductid?.review?.[10] ? getbyProductid?.review?.slice(0, 10) : getbyProductid?.review}
        ...
        {getbyProductid?.review?.[10] &&
          <TouchableOpacity activeOpacity={0.5} style={styles.view_btn} onPress={() => GetAllReviewApi(1, getbyProductid?.data?._id, navigation, (data) => setLoading(data))}>
