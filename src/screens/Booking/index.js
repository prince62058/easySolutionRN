...
import { CancelBookingApi, GetAllOrdersApi, GetOrderByIdApi, OpenPhonepayApi, UpdatetransactionIdApi } from '../../redux/actions/orderAction';
...
const Booking = ({ navigation, token, CancelBookingApi, GetOrderByIdApi, UpdatetransactionIdApi, OpenPhonepayApi, userstate, orderData, GetAllOrdersApi, getuser }) => {
  ...
  const Rozarpayonline = (id, amt) => {
    ...
    RazorpayCheckout.open(options)
      .then(data => {
        // handle success
        UpdatetransactionIdApi(id, data.razorpay_payment_id, (res, data) => { setPaymentModal(res), setLoader(data) })
        GetAllOrdersApi(1)
  ...
  const Phonepayonline = (id, amt) => {
    ...
    OpenPhonepayApi(
      { orderId: id, amount: amt, mobileNumber: getuser?.phoneNumber, userId: getuser?._id },
      (cl, response) => {
                  ...
                  {orderData?.[0] ?
                    ...
                    <FlatList
                          ...
                          {item?.product && item?.product?.map((ele, index) => {
                            return (
                              <CompletedBooking
                                ...
                                status={item?.status}
                                t2={styles.border}
                                main_img={ele?.productId?.thumnail ? { uri: http2 + ele?.productId?.thumnail } : images?.no_image}
                                ...
                                successCallback={(data) => {
                                  UpdatetransactionIdApi(item._id, data?.transactionId, (res, data) => { setPaymentModal(res), setLoader(data) })
                                  GetAllOrdersApi(1)
                                  console.log("success data : ", data)
                                }}
                                // paymentPress={() => Phonepayonline(item._id, item?.orderTotal)}
                                paymentPress={() => Rozarpayonline(item._id, item?.orderTotal)}
                                // paymentPress={() => phonePe(item._id, item?.orderTotal)}
                                cancelPress={() => { setCancel(false), setModalVisible(true), setOrderId(item._id) }}
                                invoicePress={() => navigation.navigate("Invoice", { data: item, pId: ele._id })}
                      ...
                      ListFooterComponent={() => {
                        return (
                          <View>
                            {loading1 && <ActivityIndicator color={COLORS.primary} size={'large'} style={{ marginBottom: SIZES.height * .02 }} />}
                          </View>
                        )
                      }}
                    />
                    // </View>
                    :
                    <NoDataBox source={images.search_not_found} title={"Booking not found"} />
                  }
                </>
                :
                <>
                        ...
                        >
                          {item?.product && item?.product?.map((ele, index) => {
                            return (
                              <OrderCard
                                key={ele?._id}
                                source={ele?.productId?.thumnail ? { uri: http2 + ele?.productId?.thumnail } : images?.no_image}
                                title={ele?.productId?.title}
                                subtitle={ele?.productId?.subtitle}
                                orderNo={item?._id}
                                date={item.createdAt ? formattedDate3(item.createdAt) : ''}
                                // price={ele?.productId?.price}
                                price={item?.orderTotal}
                                status={ele?.status}
                                name={`${item.address?.firstName} ${item.address?.lastName}`}
                                trackPress={() => navigation?.navigate("Tracking", { status: ele?.status })}
                                onPress={() => GetOrderByIdApi(item._id, ele?._id, navigation, data => setLoader(data))}
