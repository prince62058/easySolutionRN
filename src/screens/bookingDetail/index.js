    ...
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        {singleOrder?.orderDetails?.status == 'PENDING' && (
          <TouchableOpacity
            style={[styles.t2, {backgroundColor: COLORS.primary}]}
            onPress={() => {
              const body = {
                amount: singleOrder?.orderDetails?.orderTotal,
                callbackUrl: `https://api.essindiaonline.com/api/PhonePayGatewayStatus?merchantTransactionId=1&orderId=2`, // You might want to make this dynamic
              };
              PhonepeCheckout(body, data => {
                if (data.funcStatus) {
                  UpdatetransactionIdApi(
                    singleOrder?._id,
                    data?.merchantTransactionId, // Use merchantTransactionId as it is what we check status with
                    (res, data) => {
                      setPaymentModal(res);
                      setLoading(data);
                    },
                  );
                  GetOrderByIdApi(singleOrder?._id); // You might want to pass the ID here
        ...
        {arr?.map((item, index) => (
          <OrderCard1
            ...
            paymentMode={singleOrder?.paymentMethod}
            onPress={() =>
              navigation?.navigate('BookingDetail', {pId: item?._id})
            }
            imagePress={() =>
