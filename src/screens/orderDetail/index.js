...
import { CancelBookingApi, GetOrderByIdApi, SendReturnRequestApi, UpdatetransactionIdApi } from '../../redux/actions/orderAction'
...
import { AddReviewApi, GetByProductIdApi } from '../../redux/actions/categoryAction'
// import { PhonepeCheckoutTouch } from 'react-native-phonepesdk'
const { height, width } = Dimensions.get('window');


const OrderDetail = ({ navigation, getuser, SendReturnRequestApi, AddReviewApi, route, singleOrder, CancelBookingApi, UpdatetransactionIdApi, GetOrderByIdApi, GetByProductIdApi }) => {
    // const orderData = singleOrder || null
    let product = route?.params?.pId ? singleOrder?.product?.find(i => i._id == route?.params?.pId) || null : singleOrder?.product?.[0]
    let pIndex = route?.params?.pId ? singleOrder?.product?.findIndex(i => i._id == route?.params?.pId) : 0
    ...
    const handleChange = (name, value,) => {
        ...
        if (name == 'rating') {
            handleChangeErrors(name, postData.rating ? null : "Please add your rating")
        } else {
            handleChangeErrors(name, postData.comment ? null : "Please add your comment")
        }
    }
    ...
    const handleRazorpayPayment = (id) => {
        ...
        RazorpayCheckout.open(options).then((data) => {
            ...
            UpdatetransactionIdApi(singleOrder?._id, data?.razorpay_payment_id, (res, data) => {
                setPaymentModal(res);
                setLoading(data);
            });
        ...
        <View style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps={'handled'}>
                <OrderCard1
                    ...
                    imagePress={() => GetByProductIdApi(product?.productId?._id, navigation, data => setLoading(data))}
                />
                {/* {singleOrder?.orderDetails?.status == "PENDING" &&
                 ...
                 {singleOrder?.orderDetails?.status == "PENDING" &&
        <TouchableOpacity style={styles.t2} onPress={() => handleRazorpayPayment(singleOrder?._id)}>
            <Text style={styles.text16}>Pay Now</Text>
        </TouchableOpacity>
    }
    
                {singleOrder?.orderDetails?.status == "DELIVERED" &&
                    <TouchableOpacity activeOpacity={0.5} style={[styles.t2, { borderRadius: 8 }]} onPress={() => SendReturnRequestApi(product?.productId?._id, singleOrder?._id, navigation, (data) => setLoading(data))} >
                ...
                {arr?.map((item, index) => (
                    <OrderCard1
                        ...
                        imagePress={() => GetByProductIdApi(item?.productId?._id, navigation, data => setLoading(data))}
                    />
                ))}
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.row}>
                        ...
                        <Text style={styles.value}>₹{singleOrder?.orderDetails?.netAmount}</Text>
                    </View>
                    <View style={styles.row}>
                        <Text style={styles.key}>Delivery Charge</Text>
                        <Text style={[styles.value, { color: COLORS.success }]}>Free Delivery</Text>
                    </View>
                    <View style={styles.row}>
                        <Text style={styles.key}>Coupons Discount</Text>
                        <Text style={styles.value}>₹{singleOrder?.orderDetails?.totalOfferDiscount ? singleOrder?.orderDetails?.totalOfferDiscount : 0}</Text>
                    </View>
                    <View style={styles.row2}>
                        <Text style={styles.key1}>Total Amount</Text>
                        <Text style={styles.value1}>₹{singleOrder?.orderDetails?.orderTotal}</Text>
                    </View>
                </View>
                ...
                <View style={styles.card}>
                    ...
                    <Text style={styles.name}>{singleOrder?.address?.firstName} {singleOrder?.address?.lastName}</Text>
                    <Text style={styles.address}>{`${singleOrder?.address?.address}, ${singleOrder?.address?.apartment}, ${singleOrder?.address?.area}, ${singleOrder?.address?.city}, ${singleOrder?.address?.landmark}, ${singleOrder?.address?.country}, ${singleOrder?.address?.state}, ${singleOrder?.address?.pinCode}`}</Text>
                    <Text style={styles.name}>Phone no: <Text style={styles.address}>{singleOrder?.address?.mobile}</Text></Text>
                ...
                <View style={styles.row1}>
                    {product?.status != "CANCELLED" &&
                        <TouchableOpacity activeOpacity={0.5} style={styles.btn1} onPress={() => navigation.navigate("Tracking", { status: product?.status })}>
                    ...
                    <View style={styles.review_box}>
                        ...
                        <View style={styles.star_row}>
                            <Stars
                                ...
                                count={5}
                                spacing={SIZES.width * .02}
                                // half={true}
                                fullStar={<Icons name={'star-fill'} size={SIZES.width * .07} color={COLORS.yellow} />}
                                emptyStar={<Icons name={'star-outline'} size={SIZES.width * .07} color={"#9B9B9B"} />}
                        ...
                        <Input1
                            ...
                            multiline={true}
                            inputTextStyle={{ width: width * .92 }}
                            numberOfLines={5}
                            value={postData.comment}
                            onChangeText={(text) => handleChange("comment", text)}
                            error={errors.comment}
                        />
                        <Button t2={{ marginBottom: SIZES.height * .02, width: width * .45 }} onPress={handleSubmitReview} t1={"Submit"} />
                    </View>
                }
            </ScrollView>
            ...
            {singleOrder?.orderDetails?.status != "PENDING" &&
                <View style={styles.cancel_row}>
                    {singleOrder?.orderDetails?.status != "PENDING" && singleOrder?.orderDetails?.status != "CANCELLED" && singleOrder?.orderDetails?.status != "RETURNED" &&
                        <>
                            ...
                            <TouchableOpacity style={[styles.t12, singleOrder?.orderDetails?.status == "ORDERED" ? { width: width * 0.43 } : { width: width * 0.9 }]} onPress={() => viewFileFromUrl(http2 + singleOrder?.invoice, "download", (data) => setLoading(data))}>
            ...
            <PaymentSuccessModal visible={paymentModal} orderId={orderId} onchangeVisible={() => {
                setPaymentModal(false)
                navigation?.navigate("Booking")
            }} />

            <OrderCancelModal visible={cancelModal} onchangeVisible={() => {
            ...
            <BottomSheet
                ...
                onBackButtonPress={() => { setCancel(false), setReason(''), setModalVisible(false) }}
                onBackdropPress={() => { setCancel(false), setReason(''), setModalVisible(false) }}
            >
                <View style={styles.modalstyle}>
                    <View>
                        <Text style={styles.text6}>{cancel ? "Reason" : "Cancel Order"}</Text>
                    ...
                    {cancel ?
                        <Input1
                            ...
                            multiline={true}
                            numberOfLines={5}
                            inputTextStyle={{ marginTop: SIZES.height * .025 }}
                            value={reason}
