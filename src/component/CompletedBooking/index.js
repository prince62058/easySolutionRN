...
const CompletedBooking = ({
  ...
  mobile,
  paymentPress,
  cancelPress, invoice, onPress,
  invoicePress,
  email, subtitle, successCallback, orderTotal,
  mb,
}) => {
  ...
  return (
    <View>
      <View style={[styles.base, mb && { marginBottom: mb }]}>
        <TouchableOpacity activeOpacity={0.6} style={styles.box1} onPress={onPress}>
          <Image source={main_img} style={styles.img1} />
          <View style={styles.box2}>
            <Text numberOfLines={1} style={styles.text1}>{text_main}</Text>
            <Text numberOfLines={2} style={styles.text2}>{subtitle}</Text>
            <View style={[styles.btn, status == "CANCELLED" && { backgroundColor: COLORS.red, }, status == "PENDING" && { backgroundColor: "#ebc034" }]}>
              <Text numberOfLines={1} style={{ ...styles.text3, marginBottom: status?.length > 10 ? 0 : -2 }}>{status}</Text>
            </View>
          </View>
        </TouchableOpacity>
        ...
        <Collapsible collapsed={isCollapsed}>
          <View>
            ...
            <View style={styles.box4}>
              ...
              <Text numberOfLines={1} style={styles.value}>{address}</Text>
            </View>
            <View style={styles.box4}>
            ...
            {status == "PENDING" ?
              ...
              //   amount={Number(1) * 100} //required orderTotal
              ...
              // >
              //   <Text style={styles.text6}>Pay Now</Text>
              // </PhonepeCheckoutTouch>
              <TouchableOpacity activeOpacity={0.5} style={[styles.t2,]} onPress={paymentPress}>
                <Text style={styles.text6}>Pay Now</Text>
              </TouchableOpacity>
              :
              <View style={styles.cancel_row}>
                ...
                {status == "CANCELLED" ?
                  ...
                  :
                  invoice &&
                  <TouchableOpacity activeOpacity={0.5} style={[styles.t2, status != "CANCELLED" && { width: width * 0.43 }]}
        ...
        <TouchableOpacity activeOpacity={0.5} onPress={() => setisCollapsed(!isCollapsed)} style={styles.collapse_btn}>
          <Icons name={isCollapsed ? 'down-outline' : "up-outline"} size={width * 0.07} color={COLORS.primary} />
        </TouchableOpacity>
      </View >
    </View>
  );
